...
function createPhraseBox(endpoint) {
  ...
  document.querySelector('#truth_button').addEventListener('click', () => {
    ...
    if (NSFW_enabled) {
      createPhraseBox('http://gesugao.me:1234/get_ANY_truth');
    }

    if (!NSFW_enabled) {
      createPhraseBox('http://gesugao.me:1234/get_SFW_truth');
    }
  });
  ...
  document.querySelector('#dare_button').addEventListener('click', () => {
    ...
    if (NSFW_enabled) {
      createPhraseBox('http://gesugao.me:1234/get_ANY_dare');
    }

    if (!NSFW_enabled) {
      createPhraseBox('http://gesugao.me:1234/get_SFW_dare');
    }
  });
